...
const DATA_FILE = path.resolve(__dirname, "just-my-look-viral.json");
 ...
 */
async function scrapeProduct(page: Page, productUrl:string): Promise<ProductData | null> {
  for (let attempt = 1; attempt <= RETRY_COUNT; attempt++) {
    try {
      ...
      console.error(`Attempt ${attempt} failed for ${productUrl}:`, error instanceof Error ? error.message : String(error));
...
async function scrapeProductsBatch(
  ...
  for (let i = 0; i < productUrls.length; i += CONCURRENCY) {
    ...
    console.log(`  Processing batch ${Math.floor(i / CONCURRENCY) + 1}/${Math.ceil(productUrls.length / CONCURRENCY)} (${batch.length} products)`);
...
async function justMyLookScraper() {
  ...
  try {
    ...
    for (let pageNumber = 1; ; pageNumber++) {
      console.log(`\n=== Processing Collection Page ${pageNumber} ===`);
      const collectionUrl = `${BASE_URL}/collections/korean-skincare-view-all?page=${pageNumber}`;
      ...
      await mainPage.goto(collectionUrl, { waitUntil: 'networkidle2', timeout: PAGE_TIMEOUT });

      // Get all product links that have one of the target badges
      const productLinks = await mainPage.evaluate((targetBadges: string[]) => {
        ...
        productCards.forEach((card) => {
          ...
          if (hasTargetBadge) {
            const linkElement = card.querySelector(".card__heading a") as HTMLAnchorElement;
            if (linkElement?.href) {
      ...
      if (productLinks.length === 0) {
        console.log("No more products with target badges found. Ending pagination.");
