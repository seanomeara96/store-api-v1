...
function card(data) {
  const { headline, condition, full_img, destination_url } = data;
  return `<div 
  style="
  ...
  <div
    ...
    ">
      <h2>${headline}</h2>
      <p style="font-size: 1.2rem">${condition}</p>
      <div>
        <a
          style="
            border: none;
...
function main(cmd) {
  body[`insertAdjacent${cmd ? "HTML" : "Text"}`](
    "beforeend",
    `<div style="margin-bottom: 50px;">${html}</div>`
  );
}
