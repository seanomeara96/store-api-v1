...
getAllOrders({
  ...
  .then(async (orders) => {
    try {
      ...
      console.log(`${rejected.length} promises rejected / ${res.length} total responses`)
      ...
      const output = await ouput(`${period.monthName}-couponUsage`, couponRank);
      const attachment = Buffer.from(output).toString("base64");
      const msg = {
