...
} from "../../functions/products/getAllProducts";
...
async function alignSO() {
  try {
    const srcStore = "ch";
    const destStore = "ha";
    const srcCategory: number | undefined = undefined;
    ...
    if (srcCategory) {
      params["categories:in"] = [srcCategory].join(",");
    }
    const src_products = await getAllProducts(params);

    const src_data: { sku: string; sort_order: number }[] = [];
    for (const v of src_variants) {
      for (const product of src_products) {
        if (srcCategory && !product.categories.includes(srcCategory)) continue;
